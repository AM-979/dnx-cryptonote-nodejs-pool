...
function handleServerRequest (request, response) {
	...
	switch (urlParts.pathname) {
		...
		case '/miners_hashrate':
			...
			break;
		case '/miners_hashrate2':
			if (!authorize(request, response)) {
				return;
			}
			handleGetMinersHashrate2(response);
			break;
...
function collectStats () {
	...
	//	default value
	const hashrateWindow = config.api.hashrateWindow ? config.api.hashrateWindow : 600;
	const largeHashrateWindow = config.api.largeHashrateWindow ? config.api.largeHashrateWindow : 1800;
	const now = Date.now() / 1000;
	// let windowTime = ((now - hashrateWindow) | 0).toString();
	let largeWindowTime = ((now - largeHashrateWindow) | 0).toString();

	// ['zremrangebyscore', `${config.coin}:hashrate`, '-inf', '(' + largeWindowTime],
	let redisCommands = [
		['zremrangebyscore', `${config.coin}:hashrate`, '-inf', '(' + largeWindowTime],
	...
	async.parallel({
		...
		pool: function (callback) {
			redisClient.multi(redisCommands).exec(function (error, replies) {
				...
				// new var
				const timeThreshold = (now - hashrateWindow) * 1000;
				let miners = {
					prop: [],
					solo: []
				}
				...
				for (const string of hashrates) {
					const arr = string.split(':');
					//	1413983:XwogYqtzwbohV2psj9Dazi3PkRKy2zCKs2zrDwoYBioUAF2uL4YWNDNTDgnM4X3qjsZrgfgojbnGEZRVepMAQYce28bdtQv54~x:1690643792301:prop
					const diff = parseInt(arr[0]);
					const address = arr[1];
					const timestamp = +arr[2];
					const type = arr[3];

					let miner = miners[type][address] ? miners[type][address] : {
						diffSum: 0,
						diffSumAvg: 0
					};

					miner.diffSumAvg += diff;

					if (timestamp >= timeThreshold) {
						miner.diffSum += diff;
					}
					miners[type][address] = miner;
				}

				let totalSharesObj = { prop: 0, solo: 0 };
				let totalWorkersObj = { prop: 0, solo: 0 };
				let totalMinersObj = { prop: 0, solo: 0 };

				for (const type in miners) {
					for (const address in miners[type]) {
						const obj = miners[type][address];
						if (address.indexOf('~') !== -1) {
							++totalWorkersObj[type];
							totalSharesObj[type] += obj['diffSum'];
						} else {
							++totalMinersObj[type];
						}

						minerStats[address] = {
							hashrate: Math.round(obj['diffSum'] / hashrateWindow),
							hashrateAvg: Math.round(obj['diffSumAvg'] / largeHashrateWindow),
						};
						minersHashrate[address] = Math.round(obj['diffSum'] / hashrateWindow);
						minersAvgHashrate[address] = Math.round(obj['diffSumAvg'] / largeHashrateWindow);
					}
				}

				data.miners = totalMinersObj['prop'];
				data.workers = totalWorkersObj['prop'];
				let totalShares = totalSharesObj['prop'];
				data.hashrate = Math.round(totalShares / hashrateWindow);

				data.minersSolo = totalMinersObj['solo'];
				data.workersSolo = totalWorkersObj['solo'];
				let totalSharesSolo = totalSharesObj['solo'];
				data.hashrateSolo = Math.round(totalSharesSolo / hashrateWindow);
		...
		lastblock: function (callback) {
			getLastBlockData(function (error, data) {
				...
				// redisClient.hset(config.coin + ':stats', 'difficulty', data.difficulty, function (error, value) {}); //MPH	//	DNX
				callback(error, data);
			});
		},
...
function handleGetMinersHashrate2 (response) {
	let data = {};
	for (let miner in minerStats) {
		if (miner.indexOf('~') !== -1) continue;
		data[miner] = minerStats[miner];
	}
...
function startRpcMonitoring (rpc, module, method, interval) {
	setInterval(function () {
		rpc(method, {}, function (error, response) {
			...
			if (module === "daemon" && response && response.block_header
				&& response.block_header.difficulty && response.block_header.height) {
				redisCommands.push(['hset', config.coin + ':stats', "difficulty", response.block_header.difficulty]);
				redisCommands.push(['hset', config.coin + ':stats', "height", response.block_header.height]);
			}
