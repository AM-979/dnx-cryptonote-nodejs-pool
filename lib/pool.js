...
let poolServiceHost = 'poolservice.dynexcoin.org';
let poolServicePort = 8080;
if (config.poolService && config.poolService.host && config.poolService.port) {
	poolServiceHost = config.poolService.host;
	poolServicePort = config.poolService.port;
}
...
function processShare (miner, job, blockTemplate, params, sendReply) {
	...
	if (algo === "dynexsolve230"){
		...
		microservice(poolServiceHost, poolServicePort,'validate224', toValidate, function(error, reply){
			if (reply != 200) {
