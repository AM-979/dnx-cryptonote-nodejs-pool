...
let mallobHost = 'poolservice.dynexcoin.org';
let mallobPort = 8080;
if (config.mallob && config.mallob.host && config.mallob.port) {
	mallobHost = config.mallob.host;
	mallobPort = config.mallob.port;
}
...
function processShare (miner, job, blockTemplate, params, sendReply) {
	...
	if (algo === "dynexsolve230"){
		...
		microservice(mallobHost, mallobPort,'validate224', toValidate, function(error, reply){
			if (reply != 200) {
