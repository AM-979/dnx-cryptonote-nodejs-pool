...
let startTime = Date.now(); //MPH
let poolHashrate = 0; //MPH
let poolDifficulty = 0; //MPH
...
let MPHEnabled=config.maxPoolHashrate.enabled ? config.maxPoolHashrate.enabled : false;
...
setInterval(function () {
	if (MPHEnabled){
		redisClient.hgetall(`${config.coin}:stats`, function (error,replies) {
			poolDifficulty=replies.difficulty;
			poolHashrate=replies.hashrate;
		});
	}

}, config.maxPoolHashrate.refreshStatsInterval);
...
function Miner (rewardType, childRewardType, id, childPoolIndex, login, pass, ip, port, agent, childLogin, startingDiff, noRetarget, pushMessage) {
	...
	this.hashesRing = utils.hashesBuffer(40);
...
function updateShareResult (miner, shareAccepted, job) {
	...
	if (shareAccepted) {
		miner.hashesRing.append(Date.now(),job.difficulty);
	}
...
function handleMinerMethod (method, params, ip, portData, sendReply, pushMessage) {
	...
	switch (method) {
		case 'login':
			...
			log('warn', logSystem, 'new miner connected');
			...
			if (!validateMinerPaymentId_difficulty(login, ip, config.poolServer, 'parent ', sendReply)) {
				console.log('can`t validateMinerPaymentId_difficulty')
				return;
			}
			if (MPHEnabled){
				if ((poolDifficulty>0)&&(poolHashrate>0)){
					let uptime=(Date.now()-startTime)/1000;
					if (uptime>config.maxPoolHashrate.secondsFromStartToEnableMPH){
						let networkHashrate=poolDifficulty/config.coinDifficultyTarget;
						let allowedHashrate=networkHashrate*(config.maxPoolHashrate.maxPoolHashratePrc/100);
						if (allowedHashrate<poolHashrate){
							sendReply("Pool reached maximum hashrate. Please use another one.");
							log('warn', logSystem, 'Miner %s login rejected. Pool reached maximum allowed hashrate %s',[login,(Math.round(allowedHashrate*100)/100)]);
							return;
						}
					}
				}
		...
		case 'submit':
			...
			processShare(miner, job, blockTemplate, params, sendReply);
			break;
		case 'keepalived':
			if (!miner) {
				sendReply('Unauthenticated');
...
function recordShareData (miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate, pool) {
	...
	let workerHashrateExpireInterval = config.api.workerHashrateExpireInterval ? config.api.workerHashrateExpireInterval : 21600;
	...
	let redisCommands = [
		...
		['zadd', `${coin}:hashrate:${rewardType}:${login}`, dateNowSeconds, [job.difficulty, workerName, dateNow].join(':')],
		['expire', `${coin}:hashrate:${rewardType}:${login}`, workerHashrateExpireInterval],
...
function processShare (miner, job, blockTemplate, params, sendReply) {
	...
	if (algo !== "dynexsolve230") {
		log('error', logSystem, 'pool.js:processShare wrong algorithm from miner (algo) %s@%s', [miner.login, miner.ip]);
		updateShareResult(miner, false, job);
		sendReply('Rejected share: wrong miner version');
		return;
	}

	if (params.mallob == "") {
		log('error', logSystem, 'pool.js:processShare No mallob-id from miner %s@%s - banning', [miner.login, miner.ip]);
		updateShareResult(miner, false, job);
	...
	if (hashDiff.lt(job.difficulty)) {
		...
		updateShareResult(miner, false,job);
		sendReply('Rejected share: low difficulty');
		return;
	}
	...
	let checkhash = [convertedBlob.toString('hex'), params.result];
	let ttd = 1677762000 * 1000;
	let now=Date.now();
	// let newport=8023; //port of new poolservice-16
	let hashrate = miner.hashesRing.hr(Date.now(),job.difficulty);

	const host = 'localhost';
	let newport= 19333; //port of new poolservice-16
	if (algo === "dynexsolve230"){

		// if (algo=="dynexsolve224final"){
		let toValidate = [params.mallob, miner.login, convertedBlob.toString('hex'), params.result, hashDiff.toString(), blockTemplate.difficulty.toString(), job.height.toString(), hashrate.toString() ,config.pool_uuid, params.POUW_DATA];
		// microservice('poolservice.dynexcoin.org',8080,'validate224', toValidate, function(error,reply){
		// microservice(host, newport,'validate', toValidate, function(error, reply){
		microservice(host, newport,'validate224', toValidate, function(error, reply){
		// microservice('poolservice.dynexcoin.org',8080,'validate224', toValidate, function(error,reply){
			if (reply != 200) {
				log('error', logSystem, '[microservice] [validate] rejected hash %s@%s~%s reason: %s', [miner.login, miner.ip, miner.workerName, reply]);
				updateShareResult(miner, false,job);
				sendReply('Invalid share Err#' + reply);

				return;
			}
			// Valid share
			shareType = 'valid';
			updateShareResult(miner, true,job);
			...
			if (hashDiff.ge(blockTemplate.difficulty)) {
				apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function (error, result) {
					if (error) {
						log('error', logSystem, '[processShare] [224final] api-interfaces Error submitting block at height %d from %s@%s, share type: "%s" - %j mallob-id=%s', [job.height, miner.login, miner.ip, shareType, error, params.mallob]);
						updateShareResult(miner, false,job);
						return false;
					} else {
						let blockFastHash = utils.cnUtil.get_block_id(shareBuffer, cnBlobType).toString('hex');
						log('info', logSystem,'[processShare] [224final] Block %s found at height %d by miner %s@%s - submit result: %j',[blockFastHash.substr(0, 6), job.height, miner.login, miner.ip, result]);
...
function microservice (host, port, method, params, callback) {
